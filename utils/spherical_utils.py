""" Spherical harmonics transforms/inverses and spherical convolution.
    ...
    Source: https://github.com/daniilidis-group/spherical-cnn
"""
import functools
...
def sph_conv_batch(f, g,
    ...
    if not spectral_input:
        cf = sph_harm_transform_batch(
            f, method, harmonics_or_legendre, m0_only=False)
    else:
        cf = f
    if not spectral_filter:
        cg = sph_harm_transform_batch(
            g, method, harmonics_or_legendre, m0_only=True)
    else:
        cg = g
    ...
    na = np.newaxis
    # per degree factor
    factor = (2*np.pi*np.sqrt(4*np.pi / (2*np.arange(n/2)+1))
              )[na, na, :, na, na, na]
